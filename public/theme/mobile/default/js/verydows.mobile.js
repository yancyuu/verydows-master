...
(function ($) {
    ...
    $.fn.vdsTouchSlider = function (options) {
        ...
        if (opts.autoplay) autoTimer = setInterval(function () {
            autoPlay()
        }, 4000);
    ...
    $.asynInter = function (url, dataset, success, type, datatype) {
        $.ajax({
            type: type || 'post', dataType: datatype || 'json', url: url, data: dataset, beforeSend: function () {
                $.vdsLoading(true)
    ...
    $.asynList = function (url, dataset, success) {
        $.ajax({
            type: 'post', dataType: 'json', url: url, data: dataset, beforeSend: function () {
                $('body').append('<div class="loadbar" id="vdsbomloader"><p>正在加载</p><i class="rec-loading"></i></div>');
            }, success: function (data) {
                $('#vdsbomloader').remove();
                success.call($(this), data);
            }, error: function (data, err) {
