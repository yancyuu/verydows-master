...
//弹出收件人列表
function popCsnList() {
    $('#csnli').show().animate({left: 0}, 200, function () {
        $('#wrapper').hide()
    });
}
...
function hideCsnForm() {
    ...
    $('#csnform').animate({left: '100%'}, 200, function () {
        $(this).hide()
    });
}
...
function hideCsnList() {
    $('#wrapper').show();
    $('#csnli').animate({left: '100%'}, 200, function () {
        $(this).hide()
    });
}
    ...
    if (checkCsnForm(form)) {
        $.asynInter(api, form.serialize(), function (res) {
            if (res.status == 'success') {
                ...
                res.data.province = form.find('select[name="province"] option').not(function () {
                    return !this.selected
                }).text();
                res.data.city = form.find('select[name="city"] option').not(function () {
                    return !this.selected
                }).text();
                res.data.borough = form.find('select[name="borough"] option').not(function () {
                    return !this.selected
                }).text();
                res.data.checked = 0;
