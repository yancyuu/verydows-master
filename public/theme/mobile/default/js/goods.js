$(function () {
    $('#buyopts dl.ck a').on('click', function () {
        ...
        $('#buyopts a.cur').each(function (e) {
            addedPrice += +$(this).data('price');
        });
...
function albumSlider() {
    var container = $('#gims'), slider = container.find('.slider'), _item = slider.find('a'),
        totals = slider.find('a').length;
    var sx, mx = 0;
    var iw = _item.width();
...
function flyToCartAnimate() {
    ...
    var flyH = $(window).height() / 10 < 50 ? 50 : $(window).height() / 10,
        flyM = Math.abs((locate.offset().left - dest.offset().left) / 2);
    circle.animate({
        width: '16px',
        height: '16px',
