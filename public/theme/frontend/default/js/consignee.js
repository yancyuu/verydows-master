function checkConsigneeForm(container) {
    var form = $('#' + container);
    form.find('input[name="receiver"]').vdsFieldChecker({
        rules: {
            required: [true, '收件人不能为空'],
            maxlen: [20, '收件人不能超过20个字符']
        }
    });
    ...
    form.find('input[name="address"]').vdsFieldChecker({
        rules: {
            required: [true, '详细地址不能为空'],
            maxlen: [240, '详细地址不能超过240个字符']
        }
    });
...
function getAreaSelect(changed) {
    ...
    if (changed == 'province') {
        ...
        $('#areaslt-borough').children().not(':first').remove();
...
function presAreaSelect(container, dataset, selected) {
    $.getJSON(areaApi, dataset, function (data) {
        var opts = '';
        for (i in data) {
            if (selected == i) {
                opts += "<option value='" + i + "' selected='selected'>" + data[i] + "</option>";
            } else {
