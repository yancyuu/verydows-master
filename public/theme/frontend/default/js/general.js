...
function viewTopUserbar() {
    ...
    if (username != null) {
        container.html(juicer($('#logined-userbar-tpl').html(), {
            username: username,
...
(function ($) {
    $.fn.vdsFieldChecker = function (options) {
        ...
        if (opts.tipsPos == 'abs') {
            ...
            tips.css({
                ...
                position: 'fixed',
                left: field.offset().left + field.outerWidth() + 5,
                top: field.offset().top - $(document).scrollTop(),
                'z-index': 9999
            });
    ...
    $.fn.vdsConfirm = function (options) {
        ...
        $confirm.css({
            left: btn.offset().left - $confirm.width() + opts.left,
            top: btn.offset().top - btn.height() - $confirm.height() + opts.top
        }).show().find('button').off('click').on('click', function () {
            if ($(this).index() == 0) {
                opts.ok();
            } else {
                opts.no();
    ...
    $.vdsPopWaiting = function (options) {
        ...
        if (opts.sw) {
            var waiting = $('<div id="vdspopwaiting" class="popwaiting"><i></i><p>' + opts.text + '</p></div>').appendTo($('body')),
                width = waiting.width(), height = waiting.height();
            var left = ($(window).width() - waiting.outerWidth(true)) / 2,
                top = ($(window).height() - waiting.outerHeight(true)) / 2;
            $.vdsMasker(true);
            waiting.show().css({width: 0, height: 0}).animate({
                width: width,
                height: height,
                left: left,
                top: top
    ...
    $.fn.vdsArrVal = function () {
        var vals = [], obj = $(this);
        if (obj.size() > 0) obj.each(function (i, e) {
            vals[i] = $(e).val()
        });
        return vals;
    }
